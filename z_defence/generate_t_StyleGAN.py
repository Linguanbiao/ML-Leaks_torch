...
from dataset import readCIFAR10, readCIFAR100, readFLW, readMINST, readcifar10_GAN, readcifar10_StyleGAN
...
def read_cifar10StyleGAN():

   testx_, testy_ = [], []

   for label in range(10):
       data = np.load('../data/png2npy/sample_'+str(label)+'.npy', allow_pickle=True).item()
...
if __name__ == '__main__':
    ...
    toTrainData_, toTrainLabel_ = read_cifar10StyleGAN()

    toTrainData_ = toTrainData_.astype(float)
    toTrainData=preprocessingCIFAR_GAN(toTrainData_)
    cluster=10520
    toTrainData, toTrainLabel = shuffleAndSplitData_GAN(toTrainData, toTrainLabel_, cluster)
